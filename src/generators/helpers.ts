...
import { parseType as parseTypeFn, convertToTypescript } from '../type-convector';
...
export function generateType(type: string, config: IConfig): string {
    ...
    const parsedType = typeModel ? convertToTypescript(typeModel, config) : type;
...
export function generateControllerBody(name: string, params: Array<IParseResultData>) {
...
export function trimMemberName(name: string, config: IConfig) {
    ...
    const postfixes = Array.isArray(config.trimPostfixes) ? config.trimPostfixes : [config.trimPostfixes];
    ...
    do {
        ...
        for (var index = 0; index < postfixes.length; index++) {
            ...
            if (!config.recursiveTrimPostfixes)
                return name;
