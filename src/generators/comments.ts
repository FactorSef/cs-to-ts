...
export function generateJsDoc(value: IParseResultData): string {
	...
	const isSimpleComment: IParseResultDataItem = (function () {
		if (items.length == 3) {
			const isSummaryStart = (x: IParseResultDataItem) =>
				x.type === ParseDataTypeEnum.start && x.tag === "summary";
			const isSummaryEnd = (x: IParseResultDataItem) =>
				x.type === ParseDataTypeEnum.end && x.tag === "summary";

			const [start, content, end] = items;

			if (
				isSummaryStart(start) &&
				isSummaryEnd(end) &&
				content.type === ParseDataTypeEnum.docStart
			) {
				return {
					simple: true,
					indent: start.space,
	...
	if (isSimpleComment) {
		return generateSimpleJsDoc(
			isSimpleComment.indent,
			isSimpleComment.content
		);
	}
	...
	const withSpaces = items
		.filter(function (x) {
			return x && x.space != null;
		})
		.map(function (x) {
			return x.space;
		});
