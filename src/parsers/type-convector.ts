...
export function isUriSimleType(type: ExtendsType) {
    ...
    if (isSimple(type as CSTypeEnum)) {
        return true;
    } else if (catType === CSTypeEnum.Nullable && isSimple(getTypeCategory(type.generics[0]))) {
        return true;
    }
...
export function getTypeCategory(type: ITypeModel): CSTypeEnum {
    ...
    if (byteTypeName.indexOf(type.name) !== -1 && type.generics.length === 0 && type.array.length === 1 && type.array[0].dimensions === 1) {
...
export function convertToTypescript(type: ITypeModel, config: IConfig): string {
    if (config.byteArrayToString && getTypeCategory(type) === CSTypeEnum.ByteArray) {
        return 'string';
    }
...
function convertToTypescriptNoArray(value: ITypeModel, config: IConfig): string {
    ...
    switch (category) {
        ...
        case CSTypeEnum.Dictionary: {
            const key = (getTypeCategory(value.generics[0]) === CSTypeEnum.Number) ? 'number' : 'string';
        ...
        case CSTypeEnum.Tuple: {
            const tupleElements = value.generics.map((item: ITypeModel, i: number) => {
                const type = convertToTypescript(item, config);
                return `Item${i + 1}: ${type}`;
            })

            const elements = tupleElements.reduce((acc, el) => acc ? `${acc}, ${el}` : el, '');
 ...
 */
export function splitTopLevel(text: string, separators: Array<string>, openGroup = ['[', '(', '<', '{'], closeGroup = [']', ')', '>', '}']): Array<string> {
    const result = [];
...
export function parseType(code: string): ITypeModel {
    ...
    const generics = genericsOrNull.map(function (x) { return x; });
