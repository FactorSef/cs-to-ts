...
export function parseProperty(code: string): IParseResult {
	const propAttributes = optional(
		seq(
			commas(seq(/\[/, identifier, /.*/, /\]/), spaceOrLineOptional),
			spaceOrLineOptional
		)
	);
	const propModifier = optional(
		seq(
			cap(
				seq(
	...
	const getSetOrFatArrow = (function () {
		const getSetModifier = optional(
			any(/internal/, /public/, /private/, /protected/)
		);
		...
		const initializer = optional(
			seq(spaceOptional, /=/, spaceOptional, cap(/.*/), /;/)
		);
		const getSet = seq(
			/{/,
			spaceOptional,
