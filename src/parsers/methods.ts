...
function castParsers(config: IConfig) {
	const modifier = cap(
		optional(seq(any(/public/, /private/, /protected/), spaceOrLine))
	);
	...
	const paramList = seq(
		/\(/,
		cap(optional(commas(parameter, paramSeparator))),
		/\)/
	);
	...
	const body = seq(
		spaceOrLineOptional,
		cap(neasted("{", "}", config.maxBodyDepth, false))
	);
	const method = seq(
		modifier,
		async,
	...
	const constructorCall = optional(
		seq(
			spaceOrLineOptional,
			/:/,
			spaceOrLineOptional,
			any(/base/, /this/),
			spaceOrLineOptional,
			neasted("(", ")", config.maxExpressionDepth, false)
		)
	);
	const constructor = seq(
		modifier,
		methodName,
		paramList,
		constructorCall,
