...
export function parseXmlDocBlock(code: string) {
	...
	const parseNodeTag = (function () {
		...
		return function (code: string) {
			return firstMatch(code, parsers);
		};
	})();

	function parseXml(code: string) {
		return allMatches(code, parseNodeTag);
	}

	function toXmlContent(
		code: string,
		regexData: IParseResult
	): IParseResultDataItem {
		return {
			space: undefined,
	...
	function getItems(match: RegExpMatchArray) {
		return {
			items: parseXml(match[1]).map((x) =>
				x.data ? x.data : toXmlContent(match[1], x)
			),
		} as IParseResultData;
	}
...
const commentLineBegin = seq(
	cap(seq(optional(lineJump), spaceOptional)),
	str("///"),
	spaceOptional
);
...
function parseParams() {
	...
	const nodeStart = (nodeEnd: string) =>
		seq(
			optional(commentLineBegin),
			str("<"),
			cap(identifier),
			cap(attribs),
			spaceOptional,
	...
	const parseNode = (type: ParseDataTypeEnum, nodeEnd: string) => (
		...
		parseRegex(code, nodeStart(nodeEnd), ([, space, tag, attrs]) => ({
			space,
			tag,
...
function parseNodeEnd(code: string) {
	...
	return parseRegex(code, nodeEnd, ([, space, tag]) => ({
		space,
		tag,
		type: ParseDataTypeEnum.end,
	}));
}
...
function parseEmptyNode(code: string) {
	...
	return parseRegex(code, patt, ([, space, tag]) => ({
		space,
		tag,
		type: ParseDataTypeEnum.emptyNode,
	}));
}
function parseDocStart(code: string) {
	const patt = seq(commentLineBegin, cap(zeroOrMore(/[^<\n\r]/)));
	return parseRegex(code, patt, ([, space, content]) => ({
		space,
		content,
		type: ParseDataTypeEnum.docStart,
	}));
}
