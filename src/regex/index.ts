...
*/
export function neasted(start: string, end: string, maxDepth = 0, allowZeroDepth?: boolean): RegExp {
    if (start.length != 1)
    ...
    const repeat = (s: string, n: number) => range(0, n).map(() => s).join("");
    const nDepth = (n: number) => n === 0
        ? zeroDepth
